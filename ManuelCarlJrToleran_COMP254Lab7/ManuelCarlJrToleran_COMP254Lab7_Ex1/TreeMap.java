...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
  ...
  /**
   * Lab 7 Exercise 1 - Alternative implementation
  ...
  private Position<Entry<K,V>> treeSearch(Position<Entry<K,V>> p, K key) {
    Position<Entry<K,V>> position = p;                //store p in local variable
    boolean checkExternal = isExternal(position);      //variable to hold status if p is an external node
    if(checkExternal){
      return position;                                // key not found; return the final leaf
    }
    int comp = compare(key, position.getElement());   //variable to hold comparison result of key and current element in position
    while(comp != 0){
      if(comp < 0){
        position = left(position);                    // search left subtree
      }
      else {
        position = right(position);                   // search right subtree
      }
      checkExternal = isExternal(position);           // update checkExternal with new position
      if(checkExternal){
        return position;                              // key not found; return the final leaf
      }
      comp = compare(key, position.getElement());     // compare element of new position to key
    }
    return position;
